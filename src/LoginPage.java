...
public class LoginPage implements ActionListener {
	...
	LoginPage(HashMap<String,String> loginInfoOriginal){
		...
		userpasswordIDLabel.setBounds(50,150,75,75);
		...
		registButton.setBounds(325,200,100,25);
	...
	if(e.getSource()==registButton){
		massageLabel.setForeground(Color.YELLOW);
		massageLabel.setText("okey");
		frame.dispose();
		...
		if(e.getSource()==loginButton) {
			...
			if(logininfo.containsKey(userID)) {
				if(logininfo.get(userID).equals(password)) {
					...
					massageLabel.setText("wrong password try again");
				}
